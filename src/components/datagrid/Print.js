...
const contextEnabled = Object.prototype.hasOwnProperty.call(
  React,
  "createContext"
)
const hooksEnabled =
  Object.prototype.hasOwnProperty.call(React, "useMemo") &&
  Object.prototype.hasOwnProperty.call(React, "useCallback")

const PrintContext = contextEnabled ? React.createContext({}) : null
...
export default class ReactToPrint extends React.Component {
  ...
  startPrint = target => {
    const { onAfterPrint, onPrintError, print, documentTitle } = this.props
  ...
  triggerPrint = target => {
    const { onBeforePrint, onPrintError } = this.props
  ...
  handlePrint = () => {
    ...
    const markLoaded = (resource, errorMessages) => {
      if (this.resourcesLoaded.includes(resource)) {
        this.logMessages(
          ["Tried to mark a resource that has already been handled", resource],
          "debug"
        )
        return
      }
      ...
      const numResourcesManaged =
        this.resourcesLoaded.length + this.resourcesErrored.length
    ...
    printWindow.onload = () => {
      ...
      if (domDoc) {
        ...
        if (typeof defaultPageStyle !== "string") {
          this.logMessages([
            `"react-to-print" expected a "string" from \`pageStyle\` but received "${typeof defaultPageStyle}". Styles from \`pageStyle\` will not be applied.`
        ...
        if (!isText) {
          ...
          const originalInputs = contentNodes.querySelectorAll(inputSelector)
          ...
          const originalCRs = contentNodes.querySelectorAll(checkedSelector)
          ...
          const originalSelects = contentNodes.querySelectorAll(selectSelector)
  ...
  logMessages = (messages, level = "error") => {
    const { suppressErrors } = this.props
