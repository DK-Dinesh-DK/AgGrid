...
function DataGrid(props, ref) {
  ...
  const newData = rawColumns.slice().map((item1, index) => {
    ...
    if (itemFromArr2) {
      if (item1.haveChildren) item1.width = itemFromArr2;
    }
    return item1;
  });
  ...
  function selectRow({ row, checked, isShiftClick }) {
    ...
    if (checked) {
      ...
      if (isShiftClick && previousRowIdx !== -1 && previousRowIdx !== rowIdx) {
        ...
        for (let i = previousRowIdx + step; i<rowIdx && i>rowIdx; i += step) {
          const row = rows[i];
      ...
      if (isGroupRow(row)) {
        ...
        rowElements.push(
          <GroupRowRenderer
            ...
            viewportColumns={rest.multilineHeader? regroupArray(merged) : columns}
