...
export default function HeaderFilters({ direction }) {
  ...
  function destroyFilterFunction(e) {
    // setFilterText(e.target.value)
    dataGridRef.current.api.destroyFilter()
  }
  function getFilterModelFunction(e) {
    // setFilterText(e.target.value)
    console.log('datagridRef.current :>> ', dataGridRef.current);
    console.log('dataGridRef.current.api.getFilterModel() :>> ', dataGridRef.current.api.getFilterModel());
  }
  function setFilterFunction(e) {
    // setFilterText(e.target.value)
    dataGridRef.current.api.setFilterModel({"task":"Task 2"})
  }
  return (
    <div className={rootClassname}>
      <button onClick={getFilterModelFunction} >getFilterModel</button>
      <button onClick={setFilterFunction} >setFilterModel</button>
      <button onClick={destroyFilterFunction} >destroyFilter</button>
      {/* <button
        onClick={() => {
      ...
      <button
        onClick={() => {
          // console.log(dataGridRef);
           console.log(dataGridRef.current.api.setFilterModel({ priority: "High" }));
